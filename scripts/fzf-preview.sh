#!/usr/bin/env bash
# this is mostly just the preview.sh bundled with fzf.vim, with 2 changes
# 1 - copied the CENTER/START_LINE/END_LINE logic + --line-range arg to bat from fzf.vim's tagpreview.sh
# 2 - removed windows support
...
if [ -z "$FZF_PREVIEW_COMMAND" ] && [ "${BATNAME:+x}" ]; then
  ${BATNAME} --style="${BAT_STYLE:-numbers}" --color=always --pager=never \
      --highlight-line=$CENTER --line-range="${START_LINE}:${END_LINE}" "$FILE"
  exit $?
fi
